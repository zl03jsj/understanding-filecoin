...
**data provider** 也就是SP(storage provider, miner), 在本地保存数据的索引. 它们需要发布持有数据的公告,让潜在客户知道;当数据失效时还能够撤回这些公告.
...
为了处理来自任意数量provider的任何规模的索引数据, indexer节点需要能够水平扩展,以便在indexers池中进行负载均衡. indexer池的大小需要可以动态变化.
...
一个`Index ID`总是一个从CID提取的mutihash(因为索引化的内容并没有追踪内容的编码方式), 这应该是来源于由provider托管内容的默克尔树的CID.<sub>译者注:意思是从cid转换为multihash?如何转?需要看代码?</sub>
